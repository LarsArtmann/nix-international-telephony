...
| ------------------- | ------------------------------------------------------------------------------------------------- | ------------------------------------------------------------ |
| Extension           | A SIP user account (directory entry) keyed by a 2-7 digit number; registers a phone and receives calls | `extensions` option, `extensionType` (`modules/telephony.nix`) |
...
| Gateway             | The single ITSP trunk definition: SIP proxy, credentials, outbound prefix and inbound DID          | `gateway` option, `gatewayType`, `gatewayXml`                 |
...
| DID                 | Direct Inbound Dialling number — the public number the provider sends to the `external` profile    | `gateway.did`, `public_did` dialplan extension                |
| DID destination     | Extension or ring group that answers calls to the DID                                             | `gateway.didDestination`                                     |
...
| Realm               | SIP authentication realm; for the gateway it defaults to the proxy host                            | `gateway.realm`, coturn `realm`                              |
...
| `config.js`         | Generated script handing the webphone its SIP domain and STUN/TURN ICE servers                     | `webRoot` (`modules/telephony.nix`)                          |
...
| Recording           | Per-call WAV capture via `record_session` into `/var/lib/freeswitch/recordings`                    | `recording.enable` option, `recordingActions`                |
...
| --------- | ---------------------------------------------------------- | -------------------------------------------------------- |
| Extension | A dialplan `<extension>` element — a routing *rule* (e.g. `extension_1000`, `pstn_international`) | A SIP *user* — the directory account phones register as |
| Internal  | Sofia profile serving registered users (5060/5061 + loopback WS 5066) | `internal_*` pre-processor variables in `vars.xml`        |
