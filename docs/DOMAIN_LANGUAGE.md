...
| Time window             | Optional business-hours window on a ring group (`days`, `startHour`/`endHour`); in-window the group rings, out-of-window calls divert to the after-hours destination | `ringGroups.<n>.timeWindow`, FreeSWITCH date-time condition attrs                                             |
| After-hours destination | Extension or ring group receiving calls that land outside a time window                                                                                              | `timeWindow.afterHoursDestination`, `tests/time-routing.nix`                                                  |
...
| Operator tooling baseline | Host-shell defaults shipped with the stack (monitors, dig/tcpdump/jq/sqlite/openssl, flake nix CLI pinned to the system's nixpkgs) so a deployed PBX is operable over plain ssh | `opsTools.enable` option, `modules/telephony/ops.nix`                                              |
