...
| Item | State | Gap |
| --- | --- | --- |
| Release hygiene | v0.1.0 tagged/released | tag predates the two regression fixes (`e113394`); no v0.1.1 cut, release notes don't mark v0.1.0 affected |
| Visual QA | design implemented, automated DOM checks pass | nobody has SEEN the rendered page; no screenshots, no dark/light/manual-language eyeball pass |
| BuildFlow on new repo | first run landed (`2821dfe`: managed .gitignore, dprint md reformat) and auto-pushed | run output not reviewed; `.buildflow.yml` skip/exclude semantics unverified on this repo |
| aarch64 posture | derivation cross-evals; same stance as pre-extraction | no TCG suite run for the input (unchanged limitation, but worth recording in the new repo) |
| Telephony README update.sh instruction | now says `cd ../webphone && ./package/update.sh` | assumes sibling checkout; breaks for fresh clones |
| TODO_LIST (this repo) | extraction row-material updated where touched | stale fragment "webphone app.js formatting-only review" (P31) not annotated — superseded by the move |
