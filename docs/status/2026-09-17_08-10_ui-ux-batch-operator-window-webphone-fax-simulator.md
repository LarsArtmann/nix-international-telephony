...
*Point-in-time snapshot; executed work lands in TODO_LIST/FEATURES/CHANGELOG at
harvest time (items 15–17 above). Sources of truth for this report: the git
history of this session (daemon-committed), /tmp/*check-*.log suite logs, and
the research files under ~/tmp-research/ (upstream sources cited inline).*
