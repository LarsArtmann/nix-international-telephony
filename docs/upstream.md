...
keys-only means keys-only; our flake input tracks upstream main
(v0.1.3 added VM runtime proofs, a golden `sshd -T` snapshot and new
server options such as `listenAddresses`, `usePam` and
`authenticationMethods`; main additionally emits
`PermitRootLogin "prohibit-password"` for keys-only root) and the
