...
| --- | ---- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------- | ------ | ------ | ---------------------------------------------------- |
| M1  | 1%   | Install path, user-gated: rescue-enable + power-cycle, rerun the fixed `install-pbx.sh` from evo-x2, run `push-secrets.sh`, BatchMode throughout, log to file → open — carried into the 2026-09-16 19:05 plan as P1 (owner-deferred) | Critical | 100min | G1     | Server runs the fixed closure; secrets spliced       |
| M2  | 1%   | Live-stack verification: units (freeswitch/coturn/nginx/sshd), webhook `/telnyx/webhooks/health` over HTTPS, ACME cert issuer, gateway REGED, deploy.md §5 walk → open — carried into the 19:05 plan as P2 | Critical | 45min  | M1     | The stack is actually up, not just installed         |
| M3  | 1%   | Inbound wiring: PATCH messaging-profile `webhook_url`, send+receive one SMS end to end via `/recent`, close the outbound-call loop from webhook events → open — carried into the 19:05 plan as P3 | Critical | 45min  | M2     | The 2026-09-14 open loops (SMS reply, "did it ring") |
| M4  | 1%   | First real calls + CDR: webphone register → outbound E.164; inbound to the US DID → ring group; CDR rows in Master.csv confirmed → open — carried into the 19:05 plan as P4 (consent answered 2026-09-16: record-all) | Critical | 60min  | M2     | The product works; release gate for 0.3.0            |
| M5  | 1%   | Deploy hygiene: static IPv6 + gateway in the private flake, AAAA re-add via the domains repo, delete the old (billing) server, reconcile `infra/hcloud.tf` → mostly done 2026-09-16: hcloud retired (M5.4), worktree pruned; IPv6/AAAA + old-server deletion ride P5 | High     | 50min  | M2     | One server, one truth; v6 reachable                  |
| M6  | 4%   | Initrd-audit gate: script greps the built initrd for the target platform's bus drivers (virtio for cloud, nvme/ahci otherwise); wire into the deploy flow → done 2026-09-16 — packages/initrd-audit + checks.initrd-audit | High     | 45min  | —      | The 10-second check that would have saved 09-14      |
| M7  | 4%   | Real-disk-boot VM test: boot the actual disko image via `virtualisation.diskInterface` (metal path, not the `mkForce`d stand-in root) → done 2026-09-16 — checks.telephony-metal-boot (kexec, virtio-scsi-pci) | High     | 100min | —      | Initrd/disk-driver class of bugs becomes VM-visible  |
| M8  | 4%   | Browser E2E re-run: validate the ruff `WebDriverException` narrowing live; review the failure-dump artifact question while in there → done 2026-09-16 — browser E2E re-run green (118s) | High     | 60min  | —      | The narrowed handlers survive a real session         |
| M9  | 4%   | Repo integrity audits: diff `flake.lock` inputs moved by `849e951` (revert/changelog), rerun `buildflow format` vs webphone assets + treefmt, fix/close the red Dependabot PR → done 2026-09-16 — lock audited (kept, changeloged), formatter verified, Dependabot merged | High     | 45min  | —      | No unaudited self-modifications; main CI stays green |
| M10 | 4%   | Trunk hardening: `allowedCidrs` + `restrictExternalTo` with Telnyx source nets, enable fail2ban on prod, redeploy → open — carried into the 19:05 plan as P9 | High     | 40min  | M4     | Port 5080 speaks only to the provider                |
| M11 | 20%  | Owner decision pack (one question batch, then execute answers): recording-consent posture (URGENT, gates real traffic), Warsaw/DE DIDs, Telnyx key rotation, browser-CI cadence, mainProgram policy, qemuGuest, Hetzner Cloud Firewall, CHANGELOG culture for docs/infra → done 2026-09-16 — decision pack executed (plan log 19:20) | High     | 30min  | G2     | Seven blocked rows become work or Won't-implement    |
| M12 | 20%  | Backups + alerting sink: restic/Hetzner-snapshot recipe for `/var/lib/freeswitch` + secrets dir (encrypted target only), OnFailure routing wired to a notifier → done 2026-09-16 — backups.* + alerts.* shipped, checks.telephony-backup green, pbx-prod wired | Medium   | 60min  | —      | Single-copy data stops being a data-loss risk        |
| M13 | 20%  | Scrub-checklist script: real-value greps (DIDs ±`+`/spaces, personal numbers, usernames, IPs, key IDs) over tree AND `git log --all -S`; wire into pre-commit → done 2026-09-16 — scripts/scrub-check.sh shipped, armed, and the history rewrite executed (d7ac48f) | Medium   | 45min  | —      | The 09-03 personal-data leak class gets a gate       |
| M14 | 20%  | BuildFlow noise decisions in `.buildflow.yml`: per-tool excludes (jscpd/lychee keep webphone assets), bandit B101/B108/B311 skips, vulture whitelist, pytest-test decision, `todo_min_severity`; probe whether lychee reads `lychee.toml` → done 2026-09-16 — noise decisions in .buildflow.yml/lychee.toml (AGENTS.md records them) | Medium   | 60min  | —      | Detect-only noise stops masking real signal          |
| M15 | 20%  | AGENTS.md headroom: migrate long-form entries to `docs/session-craft.md` with one-line pointers; add the `resolved =` annotation + archive rule → done 2026-09-16 — AGENTS.md halved to docs/lessons/ + the archive rule recorded | Medium   | 45min  | —      | The 377-line cap stops eating the next lesson        |
| M16 | 20%  | Release 0.3.0 after the first real call: cut `[Unreleased]`, date it, tag, `gh release create`, repo metadata refresh → open — release 0.3.0 stays gated on the first real call (19:05 plan §P18) | Medium   | 40min  | M4, G3 | Public anchor for the deployment era                 |
| M17 | rest | Docs-hygiene continuation: verdict sweep + archive of the 9 old 08-21/22 reports; resolve/mark the HTML report's open rows; decide the micro-verdict convention; DOMAIN_LANGUAGE time-window entries; deploy.md clock note; docs-drift extension (flag TODO rows citing archived files); batch `git show --stat` the ~25 hashes cited by round 2 → mostly done 2026-09-16 docs-health round: 08-21/22 sweep + archival, HTML rows, DOMAIN_LANGUAGE entries, deploy.md clock note, docs-drift archived-citation gate; hash batch-verification stays open | Low      | 100min | —      | Archives stay trustworthy; no ghost citations        |
| M18 | rest | BuildFlow ergonomics: probe `BUILDFLOW_MAX_TIME` env (shrink the AGENTS.md note if it works), document dev/fast local default, trial `watch`/`diff`/`--failed-only` once each → done 2026-09-16 — env NOT honored (flag only), fast default set, watch/diff/--failed-only exercised | Low      | 40min  | —      | Full runs stop biting; recovery path known           |
| M19 | rest | Test-depth micro pack: `assert_fs_hour` helper in `tests/common.nix`, dedupe `time-routing.nix`'s local `call()`, conference pin leg (wrong pin denied) → open — test-depth pack (TODO_LIST) | Low      | 60min  | —      | The three verified test gaps close                   |
